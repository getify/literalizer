...
(function UMD(name,context,definition) {
	...
	function tokenize(code) {
		...
		function processGeneral() {
			...
			if (unmatched) {
				...
				if (tokens[tokens.length-1] !== segments[segments.length-1]) {
					...
					if (
						...
						tokens.length > 1 &&
						// valid identifier (and surrounding whitespace)?
						unmatched.match(/^\s*[^0-9\s\(\)\[\]\{\}<>,.:;=~+\-\*\/!%&\|\?\"\'][^\s\(\)\[\]\{\}<>,.:;=~+\-\*\/!%&\|\?\"\']*\s*$/)
					) {
						tok_idx = findUsefulToken(tokens.length-2);
						if (
							// found previous useful token was a { ?
							tok_idx !== false &&
							// previous useful token was a { ?
							tokens[tok_idx].val === "{" &&
							// { was start of a statement block?
							tokens[tok_idx].is_block
						) {
			...
			if (match) {
				left_context = code.substring(0,next_match_idx - match[0].length);
				...
				if (!left_context || left_context.match(not_escaped_pattern)) {
					...
					// omit escaped new-lines including their \ escape character
						...
						segments[segments.length-1].val =
							segments[segments.length-1].val.substring(
								0,
