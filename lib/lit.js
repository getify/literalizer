...
(function UMD(name,context,definition) {
	...
	function lex(code) {
		...
		function processGeneral() {
			...
			if (
				(
					(
						...
						// not yet found the opening { for the outer { } balanced pair?
			...
			if (match) {
				...
				// special token?
				else if (/^(?:\b(?:new|return|throw|delete|in|of|else|void|typeof|yield|if|do|while|for|with|case|debugger|break|continue)\b|\bfunction(?:\s*\*)?|=>|[+\-*=~!%&,\|;:\?<>\(\)\{\}\[\]]|(?:\r?\n)+)$/.test(match[0])) {
					saveText(match[0]);
					tokens.push({
						type: TOKEN_SPECIAL,
						val: match[0]
					...
					// new-line?
						...
						if (prev_tok_idx !== false) {
							if (
								(
									tokens[prev_tok_idx].type === SEGMENT_GENERAL &&
									!preceding_dot_operator
								) ||
								(
									tokens[prev_tok_idx].type === TOKEN_SPECIAL &&
									!/^(?:[:\(\[+\-*\/%&\|=,])$/.test(tokens[prev_tok_idx].val)
								) ||
								tokens[prev_tok_idx].type === SEGMENT_NUMBER_LITERAL ||
								tokens[prev_tok_idx].type === SEGMENT_STRING_LITERAL ||
								tokens[prev_tok_idx].type === SEGMENT_REGEX_LITERAL ||
								tokens[prev_tok_idx].type === SEGMENT_BACKTICK_LITERAL
							) {
					...
					// opening ( parenthesis?
						...
						if (
							...
							// not yet found the opening ( of the outer ( ) balanced pair?
	...
	var SEGMENT_GENERAL = 0,
		...
		STATE_PATTERNS = [
			// general
			/\b(?:new|return|throw|delete|in|of|else|void|typeof|yield|if|while|for|with|case|debugger|break|continue)\b|\bfunction(?:\s*\*)?|\/\/|\/\*|=>|(?:\r?\n)|[`"'+\-*\/=~!%&,\|;:\?<>\(\)\{\}\[\]]|(?:0[xX][0-9a-fA-F]+)|(?:0[oO][0-7]+)|(?:0[bB][01]+)|(?:\d+\.\d*(?:[eE][+-]?\d+)?)|(?:\.\d+(?:[eE][+-]?\d+)?)|(?:\d+(?:[eE][+-]?\d+)?)/g,
