...
(function UMD(name,context,definition) {
	...
	function lex(code) {
		...
		function processGeneral() {
			...
			if (match) {
				...
				else if (match[0].match(/^(?:(?:0[xXbBoO][0-9a-fA-F]+)|(?:\d+\.\d*(?:[eE][+-]?\d+)?)|(?:\.\d+(?:[eE][+-]?\d+)?)|(?:\d+(?:[eE][+-]?\d+)?))$/)) {
					if (
						// actually a well-formed, recognizable number?
						!isNaN(match[0]) &&
	...
	var SEGMENT_GENERAL = 0,
		...
		STATE_PATTERNS = [
			// general
			/\b(?:return|throw|delete|in|else|void|typeof|yield|function|if|while|for|with|case|debugger|break|continue)\b|\/\/|\/\*|\=\>|[`"'+\-*\/=~!%&,\|;:\?<>\(\)\{\}\[\]]|(?:0[xXbBoO][0-9a-fA-F]+)|(?:\d+\.\d*(?:[eE][+-]?\d+)?)|(?:\.\d+(?:[eE][+-]?\d+)?)|(?:\d+(?:[eE][+-]?\d+)?)/g,
